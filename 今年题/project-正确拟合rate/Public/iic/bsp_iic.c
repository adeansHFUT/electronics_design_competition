...
u8 IIC_Read_Byte(My_iic_device device, u8 ack)
{
    ...
    if (!ack)
        IIC_NAck(device);//发送nACK
    else
        IIC_Ack(device); //发送ACK   
    return receive;
}
