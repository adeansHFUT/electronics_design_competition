...
函数作用：初始化舵机频率
参数：频率.50-1000（单位是Mz）最少50, 越小精度越高
返回：无
*/

void steer_pwm_Frequency_Init(My_pwm_device_t pwm_device_t, uint16_t fre)
{
	pwm_device_t->pwm_arr = 500000/fre;                // arr越大，pwm精度就越高（最高也就千分之一，下面的函数限制了）
	pwm_device_t->pwm_ccr = pwm_device_t->pwm_arr*0;  // 保证60000*1200 = 70M就可以了
	PWM_Init(*pwm_device_t, 144-1);  // 预分频
}
/*
函数名：motor_pwm_Frequency_Init
函数作用：初始化电机频率
...
{
	pwm_device_t->pwm_arr = 10000/fre;                // arr越大，pwm精度就越高（最高也就2千分之一，下面的函数限制了）
	pwm_device_t->pwm_ccr = pwm_device_t->pwm_arr*0;  // 保证60000*1200 = 70M就可以了
...
{
	pwm_device_t->pwm_ccr = duty*0.0001*pwm_device_t->pwm_arr;
