...
//		rt_kprintf("sem_Banqiu_task信号量创建成功！\n\n");
	...
	/* 创建一个信号量 */
	sem_elegun= rt_sem_create("sem_elegun",/* 名字 */
                     0,     /* 信号量初始值 */
                     RT_IPC_FLAG_FIFO); /* 信号量模式 FIFO(0x00)*/
    if(sem_elegun != RT_NULL)
		rt_kprintf("sem_elegun信号量创建成功！\n\n");
	
	/* 创建一个信号量 */
	sem_elegun_autofire= rt_sem_create("sem_elegun_autofire",/* 名字 */
                     0,     /* 信号量初始值 */
                     RT_IPC_FLAG_FIFO); /* 信号量模式 FIFO(0x00)*/
...
//		rt_kprintf("Banqiu_thread线程创建失败！\n\n");
	...
	Elegun_thread =                          /* 线程控制块指针 */
    rt_thread_create( "Elegun_thread",              /* 线程名字 */
                      Elegun_fire_thread_entry,   /* 线程入口函数 */
                      RT_NULL,             /* 线程入口函数参数 */
                      512,                 /* 线程栈大小 */
                      9,                   /* 线程的优先级 */
                      20);                 /* 线程时间片 */
	...
	Elegun_autofire_thread =                          /* 线程控制块指针 */
    rt_thread_create( "Elegun_autofire_thread",              /* 线程名字 */
                      Elegun_autofire_thread_entry,   /* 线程入口函数 */
                      RT_NULL,             /* 线程入口函数参数 */
                      512,                 /* 线程栈大小 */
                      12,                   /* 线程的优先级 */
                      20);                 /* 线程时间片 */
...
//        return -1;
   ...
   if (Elegun_thread != RT_NULL)
   {
		rt_thread_startup(Elegun_thread); 
        rt_kprintf("电磁炮基础部分线程开始调度！\n\n");
